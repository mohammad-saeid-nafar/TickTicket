Feature: Add new event (ID006)
  ...
  Scenario Outline: Attempting to add a new event successfully
    When the user "<User>" attempts to create an event with name "<targetName>", description "<description>", capacity "<capacity>", cost "<cost>", address "<address>" and type "<type>"
    When the user "<User>" attempts to create a new Event Schedule starting on "<startDateTime>" and ending on "<endDateTime>"
    Then there are "<numberOfEvents>" events in the system
    Then the event is created

    Examples:
      | User    |     targetName    | description | capacity | cost |   address   |   type    |   startDateTime  |    endDateTime   |
      | johnDoe | Taylor Swift Tour | Music Tour  | 560      | 300  | Bell Center | Pop Music | 2022-12-01 20:00 | 2022-12-01 22:00 |
  ...
  Scenario Outline: Attempting to add a new event at the same time as another existing event successfully
    ...
    Examples:
      | User    |     targetName    | description | capacity | cost |   address   |   type    |   startDateTime  |    endDateTime   | numberOfEvents |
      | johnDoe | Taylor Swift Tour | Music Tour  | 560      | 300  | Bell Center | Pop Music | 2022-10-15 19:00 | 2022-10-15 22:00 |              2 |

  Scenario Outline: Attempting to add a new event with non existent event type
    When the user "<User>" attempts to create an event with name "<targetName>", description "<description>", capacity "<capacity>", cost "<cost>", address "<address>" and type "<type>"
    ...
    Examples:
      | User    | targetName | description | capacity | cost | address | type  | startDateTime    |    endDateTime   | numberOfEvents |  error                    |
      | johnDoe | US Open    | Tennis Game | 300      | 140  | Court 5 | Sport | 2022-12-01 13:00 | 2022-12-01 15:00 |              1 |  Event Type doesn't exist |
  ...
  Scenario Outline: Attempting to add a new event that already exists
    ...
    Examples:
      | User    |     targetName     | description | capacity |   address   | cost |   type    |   startDateTime  |    endDateTime   | numberOfEvents | error                    |
      | johnDoe | Justin Bieber Tour | Music Tour  | 600      | True Square | 250  | Pop Music | 2022-10-15 19:00 | 2022-10-15 22:00 |              1 | The event already exists |